...
import Hap.Runtime (Env, newEmptyEnv, run)
...
start options = case optOutputMode options of
    ...
    _ <- forkIO $ case optInputMode options of
      ...
      BatchInput -> case optInputPaths options of
        ...
        paths -> do
          ...
          env <- newEmptyEnv
          putMVar quit =<< runBatch env paths
  ...
  where
    ...
    runBatch :: Env -> [FilePath] -> IO Int
    runBatch env paths = do
...
runRepl = do
  ...
  where
    loop :: Env -> Haskeline.InputT IO Int
    loop env = do
      entry <- getInputLine "> "
      case entry of
        Nothing -> do
